 ...
 *
 * Clients can use this interface to assign the layer_surface role to
 * wl_surfaces. Such surfaces are assigned to a "layer" of the output and
 * rendered with a defined z-depth respective to each other. They may also be
 * anchored to the edges and corners of a screen and specify input handling
 * semantics. This interface should be suitable for the implementation of
 * many desktop shell components, and a broad number of other features that
 * overlay or place windows with respect to the desktop.
 ...
 * describe a layer surface.
 *
 * This interface exposes requests and events relevant for describing and
 * positioning a layer surface.
 ...
 * Popups and other user interface elements are typically at overlay. But
 ...
 *
 * The provided surface must be visible as a top-level surface when the
 * role is assigned. Layer surfaces are not visible as toplevel surfaces,
 * so if the wl_surface is not a toplevel surface, it will be treated as a
 * client error.
 */
 ...
 * @ingroup iface_zwlr_layer_shell_v1
 *
 * This request indicates that the client will not use the layer shell
...
struct zwlr_layer_surface_v1_listener {
	 ...
	 *
	 * The closed event is sent by the compositor when it wants the
	 * surface to be closed. The client should then destroy the
	 * surface.
	 */
 ...
 * compositor will display the surface centered with respect to the
 * anchors.
 *
 ...
 * (e.g. the top left corner of the output); if only one edge is
 * specified, then the anchor point will be centered on that edge.
 *
 * A single edge or corner can be specified, but not both. Setting an
 * anchor point that does not touch any edge is a protocol error. Setting
 * two edges on the same axis is a protocol error.
 */
static inline void
 ...
 * with other surfaces. This is primarily intended for surfaces using the
 * top or bottom layer, to reserve screen space. The compositor's usage
 * of this space is dependent on the value of set_anchor.
 *
 * A positive value sets the size of the exclusive zone to the given
 * value in surface-local coordinates. A value of 0 requests the
 * compositor to not reserve any space. A value of -1 requests the
 * compositor to not occlude the surface with other surfaces.
 */
static inline void
 ...
 * point, in surface-local coordinates.
 */
static inline void
 ...
 * @ingroup iface_zwlr_layer_surface_v1
 *
 * Set how keyboard events are requested for this surface.
 *
 * The keyboard_interactivity value can be one of the following:
 *
 * none: no keyboard events are requested. The compositor is free to not
 * forward keyboard events to the layer surface.
 *
 * exclusive: exclusive keyboard focus. This asks the compositor to give
 * all keyboard events to this surface and not pass them on to any other
 * surface in the same seat.
 *
 * on_demand: regular keyboard focus. The compositor will forward keyboard
 * events to the surface when it has keyboard focus.
 */
static inline void
 ...
 * surface in response to the configure event, the client must make an
 * ack_configure request sometime before the commit request, passing
 * along the serial of the configure event.
 *
 * For instance, for configure events the compositor can use the serial
 * to let the client know which configure event its acking.
 */
static inline void
 ...
 * @ingroup iface_zwlr_layer_surface_v1
 *
 * This request informs the compositor that the layer surface is to be
 * destroyed. The compositor is expected to hide the surface and then
 * send the wl_surface.unmap event.
 */
static inline void
